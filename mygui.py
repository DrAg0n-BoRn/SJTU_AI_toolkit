...
def main_window():
    ...
    tabgroup = sg.TabGroup(layout=[[sg.Tab(title="Grad-CAM Analysis", layout=tab1, expand_x=True, expand_y=False), 
                                    sg.Tab(title="Image Analysis", layout=tab2, expand_x=True, expand_y=False),
                                    sg.Tab(title="Clustering Analysis", layout=tab3, expand_x=True, expand_y=False, key="-SECRET_TAB-", visible=False)]],
    ...
    layout = [
        [sg.Column([[sg.Image(source=disabled_icon, visible=True, key="-IMG_NOTOK-", subsample=2), sg.Image(source=checkmark_icon, visible=False, key="-IMG_OK-", subsample=2)]]), 
         ...
         sg.Input(disabled=True, key="-IMG_PATH-", visible=False, enable_events=True), sg.FileBrowse(button_text="Select Image", pad=(10,20))],
